        ...
        shiny::wellPanel(
          ...
          shiny::fluidRow(
            shiny::column(
              8,
              shiny::p(
                "Use the slider to view the state of classifications at different points in time. Exports will reflect the selected timestamp.",
...
mod_overview_server <- function(id, .dir, schema, marked_docs = NULL) {
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # ===== RECENT CLASSIFICATIONS TABLE (with marking) =====
    output$recent_table <- DT::renderDataTable({
      ...
      if (!is.null(marked_docs)) {
        recent$Mark <- recent$DocID %in% marked_docs$ids
      } else {
        recent$Mark <- FALSE
      }

      # Reorder columns
      recent <- recent[, c("Mark", "DocID", "UserID", "Timestamp", "Schema")]
    ...
    shiny::observeEvent(input$clear_marks_btn, {
      if (!is.null(marked_docs)) {
        count <- length(marked_docs$ids)
        marked_docs$ids <- character(0)
        shiny::showNotification(
          paste("Cleared", count, "marked documents"),
          type = "message",
          duration = 3
        )
      }
